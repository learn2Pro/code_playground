...
class _770_BasicCalculatorIV {
    ...
    public Poly parse(String expr) {
        ...
        for (int j = 0; j < symbols.size(); ++j) { ans = combine(ans, bucket.get(j + 1), symbols.get(j)); }
...
class Poly {
    ...
    Poly add(Poly that) {
        Poly ans = new Poly();
        for (List<String> k : this.count.keySet()) { ans.update(k, this.count.get(k)); }
        for (List<String> k : that.count.keySet()) { ans.update(k, that.count.get(k)); }
        return ans;
    }
    ...
    Poly evaluate(Map<String, Integer> evalMap) {
        ...
        for (List<String> k : this.count.keySet()) {
            ...
            for (String token : k) {
                if (evalMap.containsKey(token)) { c *= evalMap.get(token); } else { free.add(token); }
            }
            ans.update(free, c);
        }
        return ans;
